"""
Created on : Sun May 15 7:32 PM 2026
...
### Restructuring The Code. With Classes
## World, Vehicle, Mission, Simulation
## For the sake of Better Optimization, Fewer Calculations.
## Each Class will do it's only task, no more mixing
## More flexible
...
class Simulation:
    ...
    def run(self, initial_state, max_time=1e8):
        ...
        while self.mission.phase_idx < len(self.mission.phases):
            ...
            # 1. Gatekeeper: Check if condition is already fulfilled BEFORE running the solver
            ...
            sol = sci.solve_ivp(
                ...
                y0=state,
                t_eval=np.linspace(t_current, t_current + period , period+1),
                method="LSODA",
                rtol=1e-6, atol=1e-9,
...
def plt_orbit2D_animation():
    global xplanet, zplanet, tout, moon_x,moon_z, xout,zout
